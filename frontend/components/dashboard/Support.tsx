...
export default function Support() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      try {
        setLoading(true);
        const response = await client.user.listSupportTickets();
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    try {
      await client.user.createSupportTicket({
      ...
      console.log('Support ticket submitted successfully! You will receive a confirmation email shortly.');
      setFormData({
        subject: '',
        description: '',
        priority: 'Medium',
      ...
      console.log('Failed to submit support ticket. Please try again.');
    } finally {
          ...
          <form onSubmit={handleSubmitTicket} className="space-y-4">
            ...
            <div className="flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowTicketForm(false)}
                className="px-4 py-2 text-gray-400 hover:text-white border border-gray-600 hover:border-gray-500 rounded-lg"
              >
                Cancel
              </button>
              <button
                ...
                className="bg-orange-500 text-white hover:bg-orange-600 px-6 py-2 font-semibold uppercase tracking-wide inline-flex items-center gap-2 rounded-lg"
              >
