...
import { Edit, Trash2, Plus, CheckCircle, XCircle, Calendar, Upload, Video, Image, Info, Loader2 } from 'lucide-react';
...
export default function EventsManager() {
  ...
  const fetchEvents = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await client.church.listAllEvents();
  ...
  const createMutation = async (params: any) => {
    const event = await client.church.createEvent(params);
    setEvents(prev => [event, ...prev]);
    return event;
  };

  const updateMutation = async (params: any) => {
    const event = await client.church.updateEvent(params);
    setEvents(prev => prev.map(e => e.id === event.id ? event : e));
    return event;
  };

  const deleteMutation = async (params: any) => {
    await client.church.deleteEvent({ id: params.id });
    setEvents(prev => prev.filter(e => e.id !== params.id));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingEvent.id) {
        await updateMutation({ ...eventData, id: editingEvent.id });
        handleUpdateSuccess();
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                ...
                <th className="text-left text-gray-400 font-semibold py-3 px-3">Location</th>
            ...
            <tbody>
              {events.map((event: any) => (
                <tr key={event.id} className="border-b border-gray-700">
                  ...
                  <td className="py-3 px-3 text-gray-300">{event.locationName || 'TBD'}</td>
