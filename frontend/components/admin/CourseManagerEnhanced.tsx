...
export default function CourseManagerEnhanced() {
  ...
  const fetchCourses = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await client.academy.listAllCourses();
  ...
  const createMutation = async (params: any) => {
    const course = await client.academy.createCourse(params);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const courseData = {
        ...
        videoUrl: editingCourse.videoUrl || undefined,
        pdfUrl: editingCourse.pdfUrl || undefined,
                ...
                <tr key={course.id} className="border-b border-gray-700">
                  ...
                  <td className="py-3 px-3">
                    <div className="flex space-x-2">
                      ...
                      <button onClick={() => handleDelete(course.id)} className="text-red-400 hover:text-red-300"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
