...
export default function PartnershipApplicationsAdmin() {
  ...
  // Mock applications data with Five-Fold ministry types
  const applications = [
    {
      id: '1',
      ...
      description: 'A community-focused church serving the Springfield area for over 50 years.',
      status: 'pending',
    ...
    },
    {
      ...
      denomination: 'Baptist',
      description: 'Spreading the Gospel through outreach and community engagement.',
      status: 'approved',
  ...
  const filteredApplications = applications?.filter(app => {
    ...
    if (searchTerm) {
      ...
      const matchesSearch =
        app.churchName?.toLowerCase().includes(searchLower) ||
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={refreshing}
            className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white px-3 py-2 flex items-center gap-2 disabled:opacity-50 rounded-lg"
          >
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-white font-semibold">{app.name || app.churchName}</h3>
                    {app.ministryType && (
                  ...
                  <div className="text-gray-400 text-sm space-y-1">
                    <p>Contact: {app.contactPerson || app.contactName}</p>
                    <p>Email: {app.email}</p>
                    {app.phone && <p>Phone: {app.phone}</p>}
                    {(app.city || app.state || app.country) && (
                      <p>Location: {[app.city, app.state, app.country].filter(Boolean).join(', ')}</p>
                    )}
                    {app.website && <p>Website: {app.website}</p>}
                    {app.denomination && <p>Denomination: {app.denomination}</p>}
                  </div>
                  <p className="text-gray-500 text-xs mt-2">
                    Applied: {new Date(app.applicationDate || app.submittedAt).toLocaleDateString()}
                  </p>
                </div>
