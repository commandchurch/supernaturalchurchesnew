              ...
              <div key={cert.id} className="bg-gray-700/50 border border-gray-600 p-4 flex items-center justify-between">
                ...
                {cert.certificateUrl ? (
                  <a
                    ...
                    className="text-sm bg-white text-black hover:bg-gray-200 px-3 py-1 font-semibold inline-flex items-center"
                  >
...
function QuickActionCard({ icon, title, description, link, linkText }: { icon: React.ReactNode; title: string; description: string; link: string; linkText: string }) {
  return (
    <div className="bg-gray-800/50 border border-gray-700 p-6">
      ...
      <a href={link} className="bg-white text-black hover:bg-gray-200 px-4 py-2 font-semibold uppercase tracking-wide text-sm">
        {linkText}
      </a>
    </div>
  );
