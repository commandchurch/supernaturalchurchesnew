...
type Tier = 'recruit' | 'private' | 'captain' | 'commando';
...
export default function OutreachCalculator() {
  ...
  const estimatedEarnings = useMemo(() => {
    const commissionRates = [0.20, 0.10, 0.05, 0.02, 0.01, 0.01, 0.01];
    let maxLevel = 4;
    if (tier === 'captain') maxLevel = 5;
    if (tier === 'commando') maxLevel = 7;
    ...
    // Note: Commando cap is per-user, this is a simplified total.
    if (tier === 'commando' && totalEarnings > 10000) {
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {(['recruit', 'private', 'captain', 'commando'] as Tier[]).map(t => (
                <button
                  key={t}
                  onClick={() => setTier(t)}
